...
class StockNewsParallel():
    ...
    def process_row(self, row, df):
        try:
            search_query = SearchQuery(row['company_name'], 'when:1d', 'en',
                                       ['boerse.de', 'ft.com', 'marketscreener.com', 'handelsblatt.com',
...
class NewsletterParallel():
    def send_newsletter_parallel(self, customers, companies, articles_df):
        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                newsletters = [Newsletter(customer, company, articles_df) for customer, company in
                               zip(customers, companies)]
