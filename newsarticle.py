    ...
    def is_relevant(self, text):
        ...
        try:
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system",
                     "content": f"Assist in evaluating the relevance of news articles for stakeholders interested in '{self.company_name}'. Evaluate if articles focus on current and upcoming developments such as new projects, partnerships, market expansions, regulatory impacts, or innovative initiatives. Exclude articles that are primarily centered around stock price predictions, technical analysis, speculative content, comparisons of stock performance of multiple companies, or retrospective summaries of past achievements. Determine relevance with 'True' for recent news about foundational business activities or 'False' for non-compliant or outdated articles."},
    ...
    def summarize_article(self, text):
        try:
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system",
                     "content": f"Efficiently distill the essence of news articles from any language into a succinct 2-3 sentence summary in English, capturing the core information and context which are relevant to the '{self.company_name}'company/stock .Return the summary."},
                    {"role": "user", "content": text},
                ],
                model="gpt-3.5-turbo",
                timeout=30,
    ...
    def make_nice_title(self, title):
        try:
            ...
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system",
                     "content": "Return a concise, captivating English headline from any given news article title in any language. If the original title is already optimal, simply return an accurate English translation."},
